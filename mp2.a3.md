...
Note: Throwing requries quick and fast mouse drag and release of the object (IN RELEASE MODE ONLY).
...
- [x] *05pts*: `Rigidbody2D` can `SetSimulationMode` to either `STATIC`, `KINEMATIC`, or `DYNAMIC`
    ...
    - [x] While selected, `1`, `2`, and `3` should switch the object to `STATIC`, `KINEMATIC`, or `DYNAMIC` respectively
        - [x] `STATIC` objects should render with a dark gray border
        - [x] `KINEMATIC` objects should render with a magenta border
        - [x] `DYNAMIC` objects should render with a blue border (same as previous)
