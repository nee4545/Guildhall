...
void OccupancyMap::Create()
{
	...
	{
		int tileX = i % m_dimensions.x;
		int tileY = i / m_dimensions.x;

		OccupancyMapNode node;
		node.coords = IntVec2( tileX , tileY );
...
void OccupancyMap::Propogate()
{

	std::vector<OccupancyMapNode> nodesToPropogate;

	for ( int i = 0; i < m_nodes.size(); i++ )
	{
		if ( !m_nodes[ i ].hasInfluence || !m_nodes[ i ].consideredForPropogation || m_nodes[ i ].value < 0.1f )
		{
			continue;
		}

		nodesToPropogate.push_back( m_nodes[ i ] );
	}



	for ( int i = 0; i < nodesToPropogate.size(); i++ )
	{
		
		IntVec2  currentcoords = nodesToPropogate[ i ].coords;
		...
		int currentIndex = GetIndexForCoords( currentcoords );
		int upIndex = GetIndexForCoords( upCoords );
		int downIndex = GetIndexForCoords( downCoords );
		int leftIndex = GetIndexForCoords( leftCoords );
		int rightIndex = GetIndexForCoords( rightCoords );

		if ( m_nodes[ upIndex ].consideredForPropogation && m_nodes[ upIndex ].hasInfluence )
		{
			m_nodes[ upIndex ].value += 1.f;
		}

		if ( m_nodes[ downIndex ].consideredForPropogation && m_nodes[ downIndex ].hasInfluence )
		{
			m_nodes[ downIndex ].value += 1.f;
		}

		if ( m_nodes[ leftIndex ].consideredForPropogation && m_nodes[ leftIndex ].hasInfluence )
		{
			m_nodes[ leftIndex ].value += 1.f;
		}

		if ( m_nodes[ rightIndex ].consideredForPropogation && m_nodes[ rightIndex ].hasInfluence )
		{
			m_nodes[ rightIndex ].value += 1.f;
		}


		m_nodes[ currentIndex ].value+=1.f;

	}
}

void OccupancyMap::PropogateInnerInfluence()
{
	for ( int i = 0; i < m_nodes.size(); i++ )
	{
		if ( !m_nodes[ i ].hasInfluence || !m_nodes[ i ].consideredForPropogation || m_nodes[i].value <= 0.01f )
		{
			continue;
		}
		...
		if ( m_nodes[ upIndex ].consideredForPropogation && m_nodes[ upIndex ].hasInfluence && m_nodes[ upIndex ].value < currentValue )
		{
			m_nodes[ i ].value += 0.25f;
		}

		if ( m_nodes[ downIndex ].consideredForPropogation && m_nodes[ downIndex ].hasInfluence && m_nodes[ downIndex ].value < currentValue )
		{
			m_nodes[ i ].value += 0.25f;
		}

		if ( m_nodes[ leftIndex ].consideredForPropogation && m_nodes[ leftIndex ].hasInfluence && m_nodes[ leftIndex ].value < currentValue )
		{
			m_nodes[ i ].value += 0.25f;
		}

		if ( m_nodes[ rightIndex ].consideredForPropogation && m_nodes[ rightIndex ].hasInfluence && m_nodes[ rightIndex ].value < currentValue )
		{
			m_nodes[ i ].value += 0.25f;
		}

		m_nodes[ i ].value = Clamp( m_nodes[ i ].value , 0.f , m_maxValue );
...
int OccupancyMap::GetIndexForCoords( IntVec2 coords )
{
	...
	return coords.x + ( m_dimensions.x * coords.y );
